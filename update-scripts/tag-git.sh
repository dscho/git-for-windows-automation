...
then
	...
	ver=${desc%%-[1-9]*} &&
	case "$ver" in
	"$desc") die "Revision '$git_rev' already tagged as $ver";;
	*.windows.*)
		tag_name=${ver%.windows.*}.windows.$((${ver##*.windows.}+1)) &&
		display_version="${ver%.windows.*}(${tag_name##*.windows.})" &&
		display_version=${display_version#v}
		;;
	*)
		tag_name=$ver.windows.1 &&
		display_version=${ver#v} &&
		if ! grep -q "^\\* Comes with \\[Git $ver\\]" "$build_extra_dir"/ReleaseNotes.md
		then
			url=https://github.com/git/git/blob/$ver &&
			txt="$(echo "${ver#v}" | sed 's/-rc[0-9]*$//').txt" &&
			url=$url/Documentation/RelNotes/$txt &&
			release_note="Comes with [Git $ver]($url)." &&
			(cd "$build_extra_dir" && node ./add-release-note.js --commit feature "$release_note")
		fi
		;;
	esac &&
